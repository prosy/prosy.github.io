...
| `combot-turkish` | A brand, running as an agent. | agentic-commerce, two-sided-marketplace, transact | AAI-02, AAI-04, LEAD-10 | full,amazon-walkthrough |
| `combot-alaska` | Live fares, real traction. | agentic-commerce, two-sided-marketplace, experiments, monetization | AAI-02, AAI-17 | full,amazon-walkthrough |
| `combot-marriott` | Hotels, as agents too. | agentic-commerce, two-sided-marketplace | AAI-02 | full |
...
- **full** (`/`): opener → surface → marketplace → agents-team → agents-deploy → agents-multi → combot-turkish → combot-alaska → combot-marriott → flights-work → checkout → hotels-search → hotels-detail → watchers → pref-declared → pref-inferred → data-ingest → loyalty → channels → control → trust → trips → social → close
- **amazon-walkthrough** (`/amazon/`): opener → surface → flights-work → marketplace → combot-turkish → combot-alaska → checkout → pref-inferred → watchers → pov-amazon → close
